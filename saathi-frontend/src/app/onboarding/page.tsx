        ...
        {DEVICES.map(d => (
          <div key={d.id}
            className={`border rounded-2xl p-4 flex items-center justify-between transition-all ${
              d.available
                ? 'border-gray-200 bg-white hover:border-blue-300 cursor-pointer group'
          ...
          >
            ...
            {d.available && (
              <span className="text-sm font-medium px-4 py-1.5 rounded-xl text-white"
...
function Step2({ onNext, onSkip }: { onNext: () => void; onSkip: () => void }) {
  ...
  return (
    <div className="max-w-lg mx-auto">
        ...
        >
          ...
          {state === 'uploading' ? (
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-sm text-gray-500">Extracting metrics from {filename}…</p>
            </div>
          ) : (
            <>
      ...
      {error && (
        <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2 mb-4">{error}</p>
      )}
...
function Step3({ onDone }: { onDone: () => void }) {
  const USECASES = [
    {
      title: 'Before every appointment',
      ...
      tag: 'Appointment Brief',
    },
    {
      title: 'When results feel confusing',
    ...
    {
      title: 'For the family member who manages everything',
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between">
        ...
        <div className="w-48"><StepBar current={step} /></div>
        <div className="flex items-center gap-4">
          <span className="text-sm text-gray-400">Step {step} of 3</span>
          <Link href="/" className="text-sm text-gray-400 hover:text-gray-600 transition-colors">Skip</Link>
        </div>
      </div>

      <div className="px-8 py-16">
        {step === 1 && <Step1 onNext={() => setStep(2)} onSkip={() => setStep(2)} />}
        {step === 2 && <Step2 onNext={() => setStep(3)} onSkip={() => setStep(3)} />}
        {step === 3 && <Step3 onDone={() => router.push('/')} />}
      </div>
    </div>
  )
}
